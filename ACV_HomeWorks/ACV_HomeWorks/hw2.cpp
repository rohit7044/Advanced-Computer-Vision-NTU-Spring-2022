...
int main()
{
    ...
    for (int i = 0; i < num_f; ++i)
    {
        for (int j = 0; j < num_object_distance; ++j)
        {
            ...
            {
                string file_path = "D:/College Works/Spring 2022/Advanced Computer Vision/ACV_HomeWorks/Image Source/";
                string img_name = "HW2/" + to_string(f[i]) + "mm/" + to_string(object_distance[j]) + "mm_" + to_string(object_displacement[k]) + "mm.jpg";
                Mat img = imread(file_path+img_name);

                if (img.empty())
                {
                    img_name = "HW2/" + to_string(f[i]) + "mm/" + to_string(object_distance[j]) + "mm_" + to_string(object_displacement[k]) + "mm.JPG";
    ...
    string ouput_path = "D:/College Works/Spring 2022/Advanced Computer Vision/ACV_HomeWorks/Image Source/HW2/HW2 Results/";
    ...
    for (int i = 0; i < num_f; ++i)
    {
        for (int j = 0; j < num_object_distance; ++j)
        {
            ...
            //Creates a Rectangular Bounding Box and let the user decide the ROI(Region of Interest)
            Rect2d img_obj = selectROI(window_name, imgDataset[i][j][0]);
            destroyWindow(window_name);
            //Fetching the value of ROI to a new image
            Mat obj = imgDataset[i][j][0](img_obj);
            // Representing the specific co-ordinate of the ROI
    ...
    // Write CSV
    ofstream output;
    output.open(ouput_path + "output.csv");
